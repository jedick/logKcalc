...
Here we add these species to a GWB thermodynamic data file and change the temperature and pressure for log*K* calculations.
...
Note that the input GWB file also has AuCl~2~^-^ and AuCl~4~^-^; the log*K* values for the former are recalculated for the given *T* and *P*, but the latter is excluded from the output because it is not available in OBIGT.
...
Both diagrams are made for a solution with 1.5 m NaCl and 0.5 m KCl and with pH set by the quartz--muscovite--K-feldspar (QMK) buffer.
To perform the calculation in GWB, we first run [this script](QMK.rea) for the React program, which uses the thermodynamic data file created above.
The first plot below shows that the minerals in the buffer are supersaturated (i.e., remain in excess) at all temperatures; the second plot shows the values of pH in the QMK buffer.
