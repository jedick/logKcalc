---
title: "Comparing GWB thermodynamic data files"
output:
  html_vignette:
    mathjax: null
vignette: >
  %\VignetteIndexEntry{1 - Comparing two GWB thermodynamic data files}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
bibliography: logKcalc.bib
csl: elementa.csl
---

<!-- https://gomakethings.com/how-to-break-an-image-out-of-its-parent-container-with-css/ -->
<style>
@media (min-width: 700px) {
  .full-width {
    left: 50%;
    margin-left: -50vw;
    margin-right: -50vw;
    max-width: 100vw;
    position: relative;
    right: 50%;
    width: 100vw;
  }
}
@media (min-width: 1020px) {
  .full-width {
    left: 50vw; /* fallback if needed */
    left: calc(50vw - 160px);
    width: 1000px;
    position: relative;
    background-color: #9ecff7;
    padding:10px;
  }
}
</style>
<script>
function myFunction() {
  // https://www.w3schools.com/howto/howto_js_toggle_hide_show.asp
  var x = document.getElementById("myDIV");
  if (x.style.display === "none") {
    x.style.display = "block";
  } else {
    x.style.display = "none";
  }
  // https://stackoverflow.com/questions/10671174/changing-button-text-onclick
  // https://stackoverflow.com/questions/7035842/how-to-change-the-buttons-text-using-javascript
  var elem = document.getElementById("myButton");
  if (elem.innerText=="Hide results") elem.innerText = "Show results";
  else elem.innerText = "Hide results";
}
function myFunction2() {
  // https://www.w3schools.com/howto/howto_js_toggle_hide_show.asp
  var x = document.getElementById("myDIV2");
  if (x.style.display === "none") {
    x.style.display = "block";
  } else {
    x.style.display = "none";
  }
  // https://stackoverflow.com/questions/10671174/changing-button-text-onclick
  // https://stackoverflow.com/questions/7035842/how-to-change-the-buttons-text-using-javascript
  var elem = document.getElementById("myButton2");
  if (elem.innerText=="Hide results") elem.innerText = "Show results";
  else elem.innerText = "Hide results";
}
</script>

```{r setup, include=FALSE}
library(logKcalc)
library(knitr)
## colorize messages 20171031
## adapted from https://gist.github.com/yihui/2629886#file-knitr-color-msg-rnw
color_block = function(color) {
  function(x, options) sprintf('<pre style="color:%s">%s</pre>', color, x)
}
knit_hooks$set(warning = color_block('magenta'), error = color_block('red'), message = color_block('blue'))
## use pngquant to optimize PNG images
knit_hooks$set(pngquant = hook_pngquant)
pngquant <- "--speed=1 --quality=0-25"
if (!nzchar(Sys.which("pngquant"))) pngquant <- NULL 
## logK with a thin space 20200627
logK <- "log&thinsp;<i>K</i>"
```

This vignette was compiled with **logKcalc** `r packageDescription("logKcalc")$Version` and **CHNOSZ** `r packageDescription("CHNOSZ")$Version`.

#### These plots compare the \logK values in thermodynamic data files from GWB and K2GWB.

The file `thermo_24noredox.tdat` was derived from the `thermo.tdat` file distributed with The Geochemist's Workbench® to write all dissociation reactions in terms of the basis species, not with redox couples.
The file `ThermoGWB_15_6_2020.tdat` was created in K2GWB [@CB05] using the same basis species.
Both files have the same 24 elements (Ag, Al, As, Au, C, Ca, Cl, Cu, F, Fe, H, K, Mg, Mn, N, Na, O, P, Pb, S, Si, Sn, U, Zn).

The logKcomp function reads the reaction coefficients and `r logK` values from the files and only considers species with the same dissociation reactions in both files.
The plot titles indicate how many of these species have `r logK` values that are not available (NA) at this particular temperature and pressure (indicated by a "500" in the GWB file).
The function adds labels to points with a difference of greater than 1 `r logK` unit.

One type of species block can be selected for comparison (aqueous, mineral, gas); here we choose minerals.
The coordinates are manually adjusted to exclude some points at extreme values, but no labels are lost; the line segments point to the points off the scale.

```{r logKcomp1-1, eval = FALSE}
file1 <- system.file("extdata/ThermoGWB_15_6_2020.tdat", package = "logKcalc")
file2 <- system.file("extdata/thermo_24noredox.tdat", package = "logKcalc")
lab1 <- "UNITHERM"
lab2 <- "thermo.tdat"
plot1 <- logKcomp(file1, file2, "mineral", 2, lab1, lab2, c(-25, 150), c(-5, 10))
```

<button id="myButton" onclick="myFunction()">Show results</button>
<div id="myDIV" style="display: none">
```{r logKcomp1-1, echo = FALSE}
```
</div>

The first plot is for 2nd *T*,*P* pair (25 °C); now we make a second plot for the 8th *T*,*P* pair (300 °C).

```{r logKcomp1-2, message = FALSE, results = "hide", fig.width = 12, fig.height = 6, out.width = "100%", out.extra='class="full-width"', pngquant = pngquant}
plot2 <- logKcomp(file1, file2, "mineral", 8, lab1, lab2, c(-25, 50))
gridExtra::grid.arrange(plot1, plot2, ncol = 2)
```

The plots reveal systematic differences between the databases: most of the large differences correspond to higher `r logK` values for the mineral dissociation reactions in `thermo.tdat`, which indicates overall lower stability of the minerals.
These differences may be associated with different estimation algorithms for thermodynamic parameters (e.g. extrapolation from high-temperature phase equilibrium data or derivation from primarily calorimetric measurements).

#### Here are some recommended commands for processing the GWB data file created by K2GWB to recalculate the `r logK` values using the [OBIGT database in CHNOSZ](../../CHNOSZ/doc/obigt.html).

The `modOBIGT` command adds more minerals from the SUPCRT92 database, an entry for steam that has been removed from OBIGT, and a modified version of realgar to match the chemical formula used in UNITHERM.

```{r logKcalc, eval = FALSE}
infile <- system.file("extdata/ThermoGWB_15_6_2020.tdat", package = "logKcalc")
outfile <- file.path(tempdir(), "ThermoGWB_OBIGT.tdat")
reset()
modOBIGT(c("addSUPCRT", "steam", "realgar*4"))
logKcalc(infile, outfile)
```

<button id="myButton2" onclick="myFunction2()">Show results</button>
<div id="myDIV2" style="display: none">
```{r logKcalc, echo = FALSE}
```
</div>

#### These plots compare the `r logK` values in the K2GWB file and OBIGT.

Only species with the same name and dissociation reaction in both files are considered for the plot.
Labels are added to points with a difference of greater than 1 `r logK` unit.

```{r logKcomp2, message = FALSE, results = "hide", fig.width = 12, fig.height = 6, out.width = "100%", out.extra='class="full-width"', pngquant = pngquant}
lab2 <- "OBIGT"
plot1 <- logKcomp(infile, outfile, "mineral", 2, lab1, lab2)
plot2 <- logKcomp(infile, outfile, "mineral", 8, lab1, lab2)
gridExtra::grid.arrange(plot1, plot2, ncol = 2)
```

There are again some differences, but the mineral data in UNITHERM and OBIGT are on average more similar to each other than to `thermo.tdat`.

## References
